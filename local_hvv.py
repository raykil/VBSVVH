...
parser.add_argument('-r', '--resume' , action="store_true", help="skip datasets already present in output/")
...
cache = Path("fetch") / f"{args.channel}.json"
if cache.exists():
    FILE_DICT = json.loads(cache.read_text())
    print(f"Using cached file list: {cache}")
else:
    FILE_DICT = {}
...
if args.resume:
    done = [k for k in FILE_DICT if (Path("output") / year_tag[SAMPLES[k]["metadata"]["year"]] / k / "out.pkl").exists()] # check if out.pkl is present
...
for dataset_name, files in FILE_DICT.items():
    ...
    # ————— Saving Output —————————————————————————
    dest = Path("output") / year / dataset_name
